...
(mymodel <- runMLwiN(log(obs) ~ 1 + perc_aff + offset(offs) + (neigh1 | 0) + (area | 1), D = "Poisson", estoptions = list(EstM = 1, 
  notation = "class", mcmcMeth = list(iterations = 50000)), data = lips1))

# 17.4 A spatial multiple-membership (MM) model . . . . . . . . . . . . .252

(mymodel <- runMLwiN(log(obs) ~ 1 + perc_aff + offset(offs) + (neigh1 | 1) + (area | 1), D = "Poisson", estoptions = list(mm = list(list(mmvar = list("neigh1", 
...
mymodel <- runMLwiN(log(obs) ~ 0 + perc_aff + offset(offs) + (area | 1) + (area | 0), D = "Poisson", estoptions = list(car = list(list(carvar = list("neigh1", 
...
(mymodel <- runMLwiN(log(obs) ~ 0 + perc_aff + offset(offs) + (area | 1) + (area | 1), D = "Poisson", estoptions = list(car = list(list(carvar = list("neigh1", 
  "neigh2", "neigh3", "neigh4", "neigh5", "neigh6", "neigh7", "neigh8", "neigh9", "neigh10", "neigh11"), weights = list("wcar1", 
  "wcar2", "wcar3", "wcar4", "wcar5", "wcar6", "wcar7", "wcar8", "wcar9", "wcar10", "wcar11")), NA, NA), EstM = 1, 
