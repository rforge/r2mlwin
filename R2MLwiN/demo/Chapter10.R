...
## Fit the model
(mymodel1 <- runMLwiN(logit(use,denomb)~(0|cons+age), levID=c('district','woman'), D="Binomial", estoptions=list(EstM=1), data=bang1))
summary(mymodel1["chains"][,"FP_age"])
sixway(mymodel1["chains"][,"FP_age"],"beta_1")

## 15,000 iterations
## Fit the model
(mymodel2 <- runMLwiN(logit(use,denomb)~(0|cons+age), levID=c('district','woman'), D="Binomial", estoptions=list(EstM=1, mcmcMeth=list(iterations=15000)), data=bang1))
sixway(mymodel1["chains"][,"FP_age"],"beta_1")
...
## Define the model
mymodel9 <- runMLwiN(logit(use,denomb)~(0|cons+age)+(2|cons), levID=c('district', 'woman'), D="Binomial",
...
(mymodel10 <- runMLwiN(logit(use,denomb)~(0|cons+age)+(2|cons), levID=c('district', 'woman'), D="Binomial", estoptions=list(EstM=1), data=bang1))
